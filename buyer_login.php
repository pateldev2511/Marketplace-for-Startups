...
if(isset($_POST['login_btn']))
{
    ...
    if(empty($username) || empty($password))
    {
     echo "<script>alert('Please Enter Your Login Credentials...!!!');</script>";   
        ...
        $status = "YES";
        $sql = "select * from $t2 where username='$username' AND password='$password'";
        $q = mysqli_query($con,$sql);
        $row = mysqli_num_rows($q);
        if($row == 1){
            $sql = "select * from $t2 where username='$username' AND password='$password' AND isverified='$status'";
                    $q = mysqli_query($con,$sql);
                    $row = mysqli_num_rows($q);
            if($row == 1)
            {
...
<body onload="change()">
   ...
   <center><p id="login_title">Log In As Buyer</p></center>
                ...
                <tr>
                 ...
                 <tr>
                    <td colspan="2">
                        <br>
                        <center>
                            <a href="forgot_pass_buyer.php" class='s_link'>Forgot Password ?</a>
                            <br><br>
                            <a href="buyer_signup.php" class="s_link">Not Account Yet ?</a>
