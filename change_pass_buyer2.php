...
$sql = "SELECT * FROM $t2 WHERE username='$un' AND token='$token'";
...
if(isset($_POST['change_pass_btn']))
{
    ...
    if(empty($pw) || empty($pw1) || empty($pw1))
    {
     echo "<script>alert('Please Enter All The Fields...!!!');</script>";   
        ...
        $sql = "SELECT * FROM $t2 WHERE username='$un' AND password='$pw'";
        $q = mysqli_query($con,$sql);
        $row = mysqli_num_rows($q);
        ...
        if($row == 1)
        {
            ...
            if($pw1 == $pw2)
            {
                $sql = "UPDATE $t2 SET password='$pw1' WHERE username='$un' AND token='$token'";
                $q = mysqli_query($con,$sql);
                $sql = "UPDATE $t2 SET token=0 WHERE username='$un'";
                $q = mysqli_query($con,$sql);
                if($q)
                {
