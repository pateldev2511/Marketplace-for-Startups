...
if(isset($_POST['forgot_pass_btn']))
{
    ...
    if(empty($un) || empty($email))
    {
        ...
        $sql = "SELECT * FROM $t2 WHERE username='$un' AND email='$email'";
        $q = mysqli_query($con,$sql);
        $row = mysqli_num_rows($q);
        
        if($row == 1)
        {
            $sql = "SELECT * FROM $t2 WHERE username='$un' AND email='$email' AND isverified='YES'";
            $q = mysqli_query($con,$sql);
            $row = mysqli_num_rows($q);
            if($row == 1)
            {
                    ...
                    <br><br>Click On The Following Button To Change Your Password.<br><div style='text-align:center;'><br><a href='http://localhost/MarketplaceForStartups/reset_pass_buyer.php?email=$email&username=$un&token=$token'><button style='width: 30%;height: 30px;background-color: black;color:white;font-size: 20px;border-radius: 20px;'>Reset Password </button> </a></div></font><p align='right'><i>Regards</i></p><p align='right'><b>-ValueMart.</b></p>";
